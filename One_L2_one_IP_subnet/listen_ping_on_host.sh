...
#
# Usage:
#   ./listen_ping_on_host.sh <host-ns> <interface>
#
# Example:
#   ./listen_ping_on_host.sh host2 eth2
#
...
if [ "$#" -ne 2 ]; then
  echo "Usage: $0 <host-ns> <interface>"
  exit 1
fi
...
sudo nsenter --net="$NS_PATH" tcpdump -i "$INTERFACE" -n -vv 'icmp or arp'
